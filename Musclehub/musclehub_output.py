...
def test(row):
    if row['fitness_test_date']==None:
        return 'B'
    else:
        return 'A'
df['ab_test_group']=df.apply(test,axis=1)    
...
def application(row):
    if row['application_date']!=None:
        return 'Application'
    else:
        return 'No Application'
        
df['is_application']=df.apply(lambda row: application(row),axis=1)
...
app_counts=df.groupby(by=['ab_test_group','is_application']).first_name.count().reset_index()
print(app_counts)
